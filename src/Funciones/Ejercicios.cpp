...
namespace Ejercicios {
    ...
    void barajar(std::stack<char> *M, std::stack<char> *C, std::stack<char> *D, std::stack<char> *T,
                 std::stack<char> *E) {
        while (!M->empty()) {
            switch (M->top()) {
                case 'C': C->push(M->top());
    ...
    void sumar(ListaPolinomios *A, ListaPolinomios *B) {
        ...
        ListaPolinomios *C = new ListaPolinomios();
    ...
    void sumarBordesMPP(MPP *matriz, int n) {
        int counter = 0;
        NodoMPP *nodo = matriz->getRow()[1]->getLeft();
        while (nodo->getCol() != 0) {
    ...
    void contarFilaConMasNodos(MPP *matriz, int n) {
        ...
        for (int i = 1; i <= n; ++i) {
            NodoMPP *nodo = matriz->getRow()[i]->getLeft();
            while (nodo->getCol() != 0) {
                aux++;
                nodo = nodo->getLeft();
            }
    ...
    void sumarInteriorYesquinas(MPP *matriz, int n) {
        ...
        for (int i = 1; i <= n; i++) {
            NodoMPP *nodo = matriz->getRow()[i]->getLeft();
            while (nodo->getCol() != 0) {
                if ((nodo->getRow() == 1 || nodo->getRow() == n) && (nodo->getCol() == n || nodo->getCol() == 1)) {
                    suma += nodo->getValor();
                }
                if (nodo->getRow() != 1 && nodo->getRow() != n && nodo->getCol() != 1 && nodo->getCol() != n) {
                    suma += nodo->getValor();
                }
                nodo = nodo->getLeft();
            }
